...
const PreprocessTweets = () => {
    const pageSize = 10
    const [page, setPage] = useState(1)
    const [tweetList, setTweetList] = useState(null)

    useEffect(() => {
        TweetController.getAll(false, true).then((response) => {
            ...
            <table className="w-full table">
                <thead className="main-color text-lg">
                    <tr>
                        ...
                        <td className="px-2">Preprocessed Tweet</td>
                    </tr>
                </thead>
                <tbody>
                    {
                        ...
                        tweetList.map((tweet, index) => {
                            ...
                            return (<tr className="hover:bg-gray-100 duration-200" key={tweet.id}>
                                ...
                                <td className="py-3 px-2">
                                    <Link to={`/preprocessed_tweet/${tweet.tweet_id}`}>{tweet.cleaned}</Link>
                                </td>
                            </tr>)
                        }
                        )
...
export default PreprocessTweets;