...
import {
  ...
  AiFillCaretLeft as Prev,
  AiFillCaretRight as Next,
} from 'react-icons/ai';
        ...
        <section className="dark:bg-gray-800 dark:text-gray-100">
          <div className="container mx-auto flex flex-col p-6">
            ...
            <div className="flex flex-row gap-x-4 mt-4">
              <div
                className="flex justify-center select-none items-center w-20 cursor-pointer hover:bg-gray-200"
                onClick={() => {
                  handleStepChange(-1);
                }}
              >
              ...
              <div className="flex flex-col col-span-full lg:text-left">
                <span className="text-xl font-bold md:text-2xl">
                  Step {stepNo + 1}: {stepLabels[stepNo]}
                </span>
                <span className="mt-4 text-xl dark:text-gray-300">
                  {tweetSteps != null && tweetSteps.steps[stepNo]}
                </span>
              </div>
              <div
                className="flex justify-center select-none items-center w-20 cursor-pointer hover:bg-gray-200"
            ...
            {(tweetSteps == null && <div className="p-8">Loading...</div>) ||
              (showAll && (
                <div className="grid grid-cols-2 mt-14 gap-y-4">
                  {stepLabels.map((label, index) => (
                    <div className="px-8 py-8">
                      <div className="flex flex-col col-span-full lg:text-left">
                        <span
                          className={`text-xl font-bold md:text-2xl ${
                            index === stepNo ? 'bg-green-200' : ''
                          }`}
                        >
                          Step {index + 1}: {label}
                        </span>
                        <span className="mt-4 text-xl dark:text-gray-300">
                          {tweetSteps != null && tweetSteps.steps[index]}
                        </span>
                      </div>
                    </div>
                  ))}
